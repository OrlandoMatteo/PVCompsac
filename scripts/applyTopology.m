function [ P, topology ] = applyTopology( config, panel )
...
power = zeros(S,timesteps);
current = zeros(S,timesteps);
voltage = zeros(S,timesteps);
...
for k = 1:1:idivide(N,S)
   for l = 1:1:S
       ...
       [power(l,:), current(l,:), voltage(l,:)] = powerProduction(squeeze(minG(position(1), position(2), :)));
   end
